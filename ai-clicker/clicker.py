...
def optimize_click_rate():
    """Automatyczna optymalizacja współczynnika kliknięć co 30 sek."""
    prev_revenue = 0
    while True:
        time.sleep(30)
        ...
        if revenue > prev_revenue:
            ai_settings["click_rate"] = min(
                ai_settings["click_rate"] + 0.03, ai_settings["max_click_rate"]
            )
        else:
            ai_settings["click_rate"] = max(
                ai_settings["click_rate"] - 0.02, ai_settings["min_click_rate"]
            )
        prev_revenue = revenue

def auto_scan_loop():
    """Generuje odsłony i kliknięcia co 5 sekund, uwzględnia tryb expensive."""
    while True:
        stats["imps"] += 1
        cr = ai_settings["click_rate"] * (2 if ai_settings["expensive_mode"] else 1)
        if random.random() < cr:
            stats["clicks"] += 1
        stats["revenue"] = round(
...
@app.route("/")
def dashboard():
    # Dashboard z kontrolkami i animacją konfetti przy co 3 kliknięciu "Magnes"
    return """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
#commandInput { width:100%; max-width:800px; height:60px; margin-top:15px; padding:5px; font-size:1em; }
...
.confetti-piece {
    position:absolute; width:10px; height:6px; background-color:red;
